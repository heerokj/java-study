package day12hw; //11. 점프를 포물선으로 그리면서 떨어지도록 코드 수정 //???????
...
public class TestMain11 extends Frame implements KeyListener{
	...
	public void keyPressed(KeyEvent e) {
		...
		if(code == 37) { //키보드 눌렀을때의 값이 37(왼쪽방향키)일경우 모든x값(x1~x6)이 마이너스! => 그것을 대입
			...
			int v =3;
			int g =1;
			int y =(int) (v*(Math.sin(30))*t-((1/2)*g*t*t)); 
			int x =(int)(v*(Math.cos(20)*t));
				y1 += y;
				y2 += y;
				y3 += y;
				y4 += y;
